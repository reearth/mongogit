...
import (
	...
	"github.com/reearth/mongogit/internal/mongotest"
...
func TestCollection_Find(t *testing.T) {
	...
	// all
	consumer0 := &SliceConsumer[Document[d]]{}
...
func TestCollection_Paginate(t *testing.T) {
	...
	pi, err := col.Paginate(
		...
		version.Eq(version.Latest.OrVersion()),
		nil,
		CursorPagination{First: lo.ToPtr(int64(2))}.Wrap(),
		consumer,
	)
	assert.NoError(t, err)
	assert.Equal(
...
func TestCollection_SaveOne(t *testing.T) {
	...
	// nonexistent version
	assert.Same(t, ErrNotFound, col.SaveOne(ctx, "x", Data{}, version.New().OrRef().Ref()))
}
...
func initCollection(t *testing.T) *Collection {
	...
	return NewCollection(c.Collection("test_" + uuid.NewString()))
}
